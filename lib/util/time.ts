...
export const now = () => r2s(Math.floor(performance.timeOrigin + performance.now()));
export const inPast = (timestamp: number) => timestamp < now();

const cleanText = (input:string, remove:string) => input.replace(remove, '').trim();
...
export const parse = (text: string) => {
    ...
    return {clean: cleanText(text, parsed.text), timestamp: r2s(date.getTime())};
}